...
must_haves:
  ...
  key_links:
    ...
    - from: "src/components/Calculator.tsx"
      to: "src/App.tsx"
      via: "onModeSelect prop forwarding"
      pattern: "onModeSelect"
    - from: "src/components/ModeToggle.tsx"
      to: "src/components/Calculator.tsx"
      via: "onModeSelect callback"
      pattern: "onModeSelect"
---
...
- Pass `onModeSelect` as a new prop to Calculator component (forwarded through to ModeToggle).
- When `appMode === 'graph'`: automatically set `graphVisible` to true (the graph should always be visible in graph mode).
- When switching TO graph mode: set `graphExpression` to the current `expression` value so the graph is not blank on first entry. This gives immediate visual feedback when the user switches to graph mode.
  ```ts
  // In handleModeSelect when mode === 'graph':
  if (expression) setGraphExpression(expression);
  ```
...
**Calculator.tsx — accept appMode prop and update ModeToggle call site:**
- Add `appMode: 'calc' | 'graph'` to CalculatorProps interface.
- Add `onModeSelect: (mode: 'simple' | 'expression' | 'graph') => void` to CalculatorProps interface.
- Update the existing `<ModeToggle>` JSX call from the old API (`mode={expressionMode} onModeChange={onModeChange}`) to the new three-prop API:
  ```tsx
  <ModeToggle
  ...
  />
  ```
- Forward `onModeSelect` from CalculatorProps down to `<ModeToggle>` — Calculator does not handle the callback, it passes it through to App.tsx.
- For now, layout changes are handled in Task 2.
...
<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: Human verification of mode toggle and layout</name>
  <what-built>
